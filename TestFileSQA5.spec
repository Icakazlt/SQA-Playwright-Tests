...
test('Fifth Task', async ({ page }) => {
  ...
  const ErrorMessage = page.getByText('No active account found with the given credentials')

  await EmailField.fill('test@test.com');
  await PassField.fill('welcome123');

  await LoginForm.getByRole('button').click();
  await expect(ErrorMessage).toHaveText('No active account found with the given credentials')
